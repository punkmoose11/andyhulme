...
games <- read_excel("C://Users/andyh/OneDrive/Documents/SPORTSDATA/Raw/msoc.xlsx",sheet = "Sheet1")
games <- as.data.frame(games) 

seeds <- read_excel("C://Users/andyh/OneDrive/Documents/SPORTSDATA/Raw/msoc.xlsx",sheet = "Sheet2")
seeds <- as.data.frame(seeds) 

games1 <- subset(games, year>=2001)
...
games1$quartet1 <- ifelse(is.na(games1$quartet1),games1$quartet2,games1$quartet1)
games1$quartet2 <- ifelse(is.na(games1$quartet2),games1$quartet1,games1$quartet2)
...
games1.x <- games1[c("year","name1","round","gameno","oteam1","oscore1","oteam2","oscore2","text")]
